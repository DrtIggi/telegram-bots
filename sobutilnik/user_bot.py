...
def text(message):
    if message.text=="Начать зарабатывать":
        try:
            ...
            bot.send_message(message.chat.id,r[0][0],reply_markup=user_markup)
        except Exception as f:
            os.system("echo " + str(f) + " >> exception")
            bot.send_message(message.chat.id, "Произошла ошибка")
        return 0
        ...
        try:
            ...
            user_markup = telebot.types.ReplyKeyboardRemove()
            r=no(message)
            bot.send_message(message.chat.id, r[0][0])
            bot.send_message(message.chat.id, r[1][0],reply_markup=user_markup)
            set_time(message)
        except Exception as f:
            os.system("echo " + str(f) + " >> exception")
            bot.send_message(message.chat.id, "Произошла ошибка")
